...
const searchFnStartText = "app.get('/api/search', rateLimit({ windowMs: 15 * 60 * 1000, max: 100 }), async (req, res) => {";
const searchFnStartIndex = content.indexOf(searchFnStartText);

if (searchFnStartIndex !== -1) {
  const nextAppGet = content.indexOf("app.get('/api/", searchFnStartIndex + searchFnStartText.length);
  // find the end of the search app.get by looking back from nextAppGet
  const endOfSearch = content.lastIndexOf("});", nextAppGet);
  
  if (endOfSearch !== -1) {
     const target = content.substring(searchFnStartIndex, endOfSearch + 3);
    ...
    try {
     ...
     console.log('Replaced by index boundaries');
  } else {
