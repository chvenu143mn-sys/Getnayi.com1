...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-surface-1/95 backdrop-blur-md border border-border-subtle p-3 rounded-xl shadow-xl gap-y-1.5 font-sans min-w-[125px]">
        {title && (
          <p className="text-[11px] font-bold text-text-secondary uppercase tracking-widest">{title}</p>
        )}
          ...
          {isPie ? (
            <div className="flex items-center gap-x-2 text-[12.5px]">
              <span 
                ...
                style={{ backgroundColor: payload[0].payload?.color || payload[0].color || 'var(--color-brand-primary)' }} 
              />
            ...
            payload.map((item: any, index: number) => (
              <div key={index} className="flex items-center gap-x-2 text-[12.5px]">
                <span 
                  className="size-2 rounded-full shrink-0" 
                  style={{ backgroundColor: item.fill || item.color || 'var(--color-brand-primary)' }} 
                />
              ...
              {data && data.Views !== undefined ? (
                <>
                  ...
                  {data.Likes !== undefined && (
                    <div className="flex items-center gap-x-2 text-[12.5px]">
                      <span className="size-2 rounded-full bg-brand-primary/60 shrink-0" />
                      <span className="text-text-secondary font-medium">Likes:</span>
                      <span className="text-text-primary font-mono font-bold">
                  ...
                  {data.Saves !== undefined && (
                    <div className="flex items-center gap-x-2 text-[12.5px]">
                      ...
                      <span className="text-text-secondary font-medium">Saves:</span>
                      <span className="text-text-primary font-mono font-bold">
...
export default function CreatorDashboard() {
  ...
  const distributionData = [
    { name: 'Likes', value: totalLikes, color: 'var(--color-brand-primary)' },
  ...
  return (
    <div className="flex-1 w-full bg-bg-base text-text-primary font-sans flex flex-col h-full bg-bg-base">
      {/* Header BAR */}
      <div className="sticky top-0 z-20 bg-bg-base/95 backdrop-blur-md pt-5 pb-0 px-0 border-b border-border-subtle">
        <div className="flex items-center justify-between px-4 pb-3">
          <div className="flex items-center gap-x-2">
            <button type="button" aria-label="button"  
              onClick={() => navigate('/profile')} 
              className="p-2 -ml-2 text-text-primary/90 hover:text-text-primary transition-colors hover:bg-surface-1 rounded-full"
            >
              <ArrowLeft className="size-5" />
            </button>
            <h1 className="text-[18px] font-bold text-text-primary tracking-tight flex items-center gap-1.5">
              Creator Studio
              <Award className="size-4 text-brand-primarymber-400" />
            </h1>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pt-5 px-4 pb-10">
        {loading ? (
          <div className="gap-y-6 animate-pulse">
            ...
            <div className="p-4 bg-surface-1 border border-border-subtle rounded-[22px] flex items-center shadow-lg">
              <div className="size-[46px] rounded-full bg-surface-2 shrink-0 mr-3.5" />
              <div className="flex-1 gap-y-2">
                <div className="h-4 bg-zinc-805 bg-surface-2 rounded w-1/3" />
                <div className="h-3 bg-zinc-850 rounded w-1/2" />
              </div>
              <div className="h-6 bg-surface-2 rounded-xl w-20" />
            </div>

            {/* Performance Ledger / Bento Grid Skeletons */}
            <div className="grid grid-cols-2 gap-3">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="p-4 bg-surface-2 border border-border-subtle rounded-2xl flex flex-col justify-between h-[120px]">
                  <div className="flex items-center justify-between">
                    <div className="h-3 bg-surface-2 rounded w-1/2" />
                    <div className="size-8 rounded-lg bg-surface-2" />
                  </div>
                  <div className="gap-y-2 mt-4">
                    <div className="h-6 bg-[#18181c] bg-surface-2 rounded w-2/3" />
            ...
            <div className="p-4 bg-surface-2 border border-border-subtle rounded-3xl shadow-lg flex flex-col">
                ...
                <div className="flex-1 flex items-end justify-between gap-x-4">
                  {[40, 70, 45, 90, 60, 85, 50].map((height, idx) => (
                    <div key={idx} className="flex-1 flex flex-col items-center gap-y-2">
                      <div className="w-full bg-surface-2/60 rounded-t" style={{ height: `${height}%` }} />
            ...
            <div className="bg-surface-2 border border-border-subtle p-5 rounded-[26px] flex items-start">
              <div className="size-10 bg-surface-2 rounded-2xl mr-4 shrink-0" />
            ...
            <div className="flex flex-col mb-6 gap-y-4">
              ...
              <div className="h-11 bg-surface-2 border border-border-subtle rounded-2xl w-full" />
              {[1, 2, 3].map((item) => (
                <div key={item} className="p-3.5 bg-surface-2 border border-border-subtle rounded-2xl flex items-center">
                  <div className="w-[50px] h-[64px] rounded-xl bg-surface-2 shrink-0 mr-4" />
                  <div className="flex-1 gap-y-2">
          ...
          <div className="gap-y-6 flex flex-col pb-10">
            {mainTab === 'overview' && (
              <>
                {/* Creator Mini Card */}
                <div className="p-4 bg-gradient-to-r from-[#17171e] to-[#151518] border border-border-subtle rounded-[22px] flex items-center shadow-lg">
              <div className="size-[46px] rounded-full overflow-hidden bg-zinc-850 shrink-0 mr-3.5 border border-border-subtle ring-2 ring-white/5 shadow-inner">
                {profile?.avatar_url ? (
                  <img src={profile.avatar_url} alt={profile.username} className="size-full object-cover" loading="lazy" decoding="async" />
                ) : (
                  <div className="size-full flex items-center justify-center text-text-primary/50 bg-surface-2 text-[14px] uppercase font-bold font-serif italic">
                    {profile?.username?.charAt(0) || 'U'}
                  </div>
                )}
              </div>
              <div className="flex-1 min-w-0 pr-2">
                <span className="text-[14px] font-semibold text-text-primary tracking-wide block truncate">
                  {profile?.username || 'Creator'}
                </span>
                <span className="text-[11.5px] font-medium text-text-secondary tracking-wide lowercase block leading-tight">
            ...
            <div className="flex flex-col gap-y-3 mb-4">
              <div className="flex items-center justify-between">
                <h3 className="text-[15.5px] font-bold text-text-primary tracking-wide flex items-center gap-2">
                  <TrendingUp className="size-[18px] text-brand-primary" />
                  Trending on Platform
                </h3>
                <button type="button" 
                  onClick={() => navigate('/trending')}
                  className="text-[12px] font-medium text-text-secondary hover:text-text-primary flex items-center transition-colors"
                >
              ...
              <div className="flex gap-x-2.5 overflow-x-auto no-scrollbar pb-1">
                {trendingTags.map((tag, i) => (
                   <button type="button" 
                     ...
                     className="px-3.5 py-1.5 bg-surface-2 border border-border-subtle rounded-full text-[12px] font-semibold text-text-primary/90 whitespace-nowrap hover:bg-brand-primary/10 hover:text-brand-primary transition-colors border-border-subtle"
                   >
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {/* Views */}
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Total Views</span>
                  <div className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/10 shrink-0 ml-1">
                    <Eye className="size-4" />
                  </div>
                </div>
                <div className="mt-4 flex flex-col">
                  <span className="text-[22px] font-bold font-sans text-text-primary tracking-tight">
              ...
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Engagement Rate</span>
                ...
                <div className="mt-4 flex flex-col">
                  ...
                  <span className="text-[10.5px] font-medium text-text-secondary mt-1 flex items-center gap-0.5 whitespace-nowrap">
              ...
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Avg Views/Video</span>
              ...
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Total Likes</span>
                  <div className="p-1.5 rounded-lg bg-brand-primary/10 text-brand-primary border border-brand-primary/10 shrink-0 ml-1">
                    <Heart className="size-4 fill-brand-primary/10" />
                  </div>
                </div>
                <div className="mt-4 flex flex-col">
                  <span className="text-[22px] font-bold font-sans text-text-primary tracking-tight">
                  ...
                  {totalLikes > 0 && (
                    <span className="text-[10.5px] font-medium text-brand-primary mt-1 flex items-center gap-0.5 whitespace-nowrap">
              ...
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Saved Videos</span>
                  <div className="p-1.5 rounded-lg bg-amber-500/10 text-brand-primarymber-500 border border-brand-primarymber-500/10 shrink-0 ml-1">
              ...
              <div className="p-4 bg-surface-2 border border-border-subtle rounded-2xl shadow-sm flex flex-col justify-between">
                <div className="flex items-center justify-between">
                  <span className="text-[12.5px] font-semibold text-text-secondary tracking-wide uppercase whitespace-nowrap">Total Comments</span>
          ...
          {mainTab === 'insights' && (
            ...
            <div className="p-4 bg-surface-2 border border-border-subtle rounded-3xl shadow-lg flex flex-col">
              <div className="flex flex-col mb-4">
                <div className="flex items-center justify-between">
                   <h3 className="text-[15px] font-bold text-text-primary tracking-wide">Interactive Performance Metrics</h3>
                </div>
                
                {/* Horizontal mini navigation for charts */}
                <div className="flex gap-2.5 mt-3.5 pt-1.5 border-t border-border-subtle">
                  <button type="button" aria-label="button"  
                    onClick={() => setActiveChartTab('trends')}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                      activeChartTab === 'trends' ? 'bg-brand-primary text-text-primary shadow-sm' : 'bg-transparent text-text-secondary hover:text-text-primary/80'
                    }`}
                  >
                    Audience growth
                  </button>
                  <button type="button" aria-label="button"  
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                      activeChartTab === 'comparison' ? 'bg-brand-primary text-text-primary shadow-sm' : 'bg-transparent text-text-secondary hover:text-text-primary/80'
                    }`}
                  >
                  ...
                  <button type="button" aria-label="button"  
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold tracking-wide transition-all ${
                      activeChartTab === 'distribution' ? 'bg-brand-primary text-text-primary shadow-sm' : 'bg-transparent text-text-secondary hover:text-text-primary/80'
                    }`}
                  >
                    Action distribution
                  </button>
              ...
              {!hasStats ? (
                <div className="h-[210px] w-full flex flex-col items-center justify-center text-text-secondary gap-y-2 border border-dashed border-border-subtle rounded-2xl">
                   <Info className="size-8 opacity-40 text-text-secondary" strokeWidth={1.5} />
                ...
                <div className="h-[220px] w-full text-text-secondary text-xs">
                      ...
                      <AreaChart data={finalTrendData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        <defs>
                          <linearGradient id="colorViews" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <stop offset="95%" stopColor="var(--color-brand-primary)" stopOpacity={0.0}/>
                          </linearGradient>
                        </defs>
                        ...
                        <Area type="monotone" dataKey="Views" stroke="var(--color-brand-primary)" strokeWidth={2} fillOpacity={1} fill="url(#colorViews)" />
                  ...
                  {activeChartTab === 'comparison' && (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={finalVideoComparison} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        ...
                        <Bar dataKey="Likes" fill="var(--color-brand-primary)" radius={[4, 4, 0, 0]} />
                        <Legend wrapperStyle={{ fontSize: '10px', marginTop: '10px' }} />
                      </BarChart>
                    </ResponsiveContainer>
                  )}
                  ...
                  {activeChartTab === 'distribution' && (
                    <div className="flex items-center h-full justify-around w-full">
                      ...
                      <div className="flex flex-col gap-y-2 text-text-secondary pl-4 pr-2 flex-1 justify-center">
                        {distributionData.map((entry, i) => (
                          <div key={i} className="flex items-center text-[11px] font-sans tracking-wide">
                            ...
                            <span className="font-medium text-text-primary/95 mr-1.5">{entry.name}:</span>
                            <span className="font-mono text-text-secondary font-bold">{entry.value >= 1000 ? (entry.value / 1000).toFixed(1) + 'K' : entry.value}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            {hasStats && topVideoByViews && topVideoByEngagement && (
              <div className="flex flex-col gap-y-3 mb-6">
                <h3 className="text-[15.5px] font-bold text-text-primary tracking-wide mb-1.5">Top Video Analytics</h3>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 pr-2 pt-0.5">
                    ...
                    <p className="text-[14px] text-text-primary font-semibold truncate mb-1">
                      {topVideoByViews.caption}
                    </p>
                    <p className="text-[11.5px] text-text-secondary leading-relaxed font-normal">
                      Generated <span className="font-bold text-text-primary">{topVideoByViews.views >= 1000 ? (topVideoByViews.views/1000).toFixed(1)+`K` : topVideoByViews.views} plays</span>. 
                      Replicating this visual style or audio trend could yield similar reach.
                    </p>
                  </div>
                  <div className="w-[42px] h-[54px] rounded-lg overflow-hidden bg-surface-1 border border-border-subtle shrink-0 opacity-80 mt-0.5">
                    {topVideoByViews.thumbnail_url ? (
                      ...
                      <div className="size-full flex items-center justify-center bg-surface-2">
                        <Play className="size-3 text-text-secondary" />
                      </div>
                    )}
                  </div>
                </div>
                ...
                {topVideoByEngagement.id !== topVideoByViews.id && (
                  <div 
                    className="bg-gradient-to-br from-[#101013] to-[#141417] border border-brand-primary/10 p-4 rounded-3xl shadow-sm flex items-start cursor-pointer transition-all active:scale-[0.99] hover:bg-white/[0.02]"
                    onClick={() => navigate(`/video/${topVideoByEngagement.id}`)}
                  >
                    <div className="p-2.5 bg-brand-primary/10 border border-brand-primary/20 text-brand-primary rounded-xl mr-3.5 shrink-0 shadow-sm">
                      <Heart className="size-[18px] fill-brand-primary/20" strokeWidth={2.5} />
                    </div>
                    <div className="flex-1 min-w-0 pr-2 pt-0.5">
                      <h4 className="text-[12px] font-bold text-brand-primary tracking-wide uppercase mb-0.5">Highest Engagement</h4>
                      <p className="text-[14px] text-text-primary font-semibold truncate mb-1">
                        {topVideoByEngagement.caption}
                      </p>
                      <p className="text-[11.5px] text-text-secondary leading-relaxed font-normal">
                        Driven by <span className="font-bold text-text-primary">{topVideoByEngagement.likes} likes</span> and <span className="font-bold text-text-primary">{topVideoByEngagement.saves} saves</span>. 
          ...
          {mainTab === 'performance' && (
            ...
            <div className="flex flex-col mb-6">
              <div className="flex items-center justify-between mb-4.5">
                <h3 className="text-[15.5px] font-bold text-text-primary tracking-wide">Video List & Video Ledger</h3>
                <span className="text-[11px] font-medium text-text-secondary font-mono">({displayVideos.length} Posts)</span>
              </div>

              {/* Search video list bar */}
              <div className="relative mb-4">
                 <Search className="size-4 text-zinc-501 text-text-secondary absolute left-4 top-1/2 -translate-y-1/2" />
                 <input 
                   type="text" 
                   placeholder="Search video ledger..." 
                   value={searchTerm}
                   onChange={(e) => setSearchTerm(e.target.value)}
                   className="w-full pl-11 pr-5 py-3 bg-surface-2 border border-border-subtle rounded-2xl text-[13.5px] text-text-primary/90 placeholder-zinc-500 focus:outline-none focus:border-brand-primary transition-colors"
                 />
              </div>

              {displayVideos.length === 0 ? (
                 <div className="py-16 text-center flex flex-col items-center justify-center bg-transparent mt-4">
                   <div className="w-16 h-16 rounded-full bg-surface-1 border border-border-subtle flex items-center justify-center mb-4">
                     <VideoIcon className="size-7 text-text-secondary" strokeWidth={1.5} />
                   </div>
                   <h3 className="text-[17px] font-bold text-text-primary tracking-tight mb-1">No data available</h3>
                   <p className="text-[13px] text-text-secondary max-w-[200px] mx-auto leading-relaxed">Upload content to see your performance metrics and insights.</p>
                 </div>
              ) : (
                 <div className="flex flex-col gap-y-3">
                   {displayVideos.map((video, index) => (
                     <div 
                       ...
                       className={`p-3.5 bg-surface-2 border border-border-subtle rounded-2xl flex items-center shadow-md transition-all hover:bg-surface-1 cursor-pointer active:scale-[0.99]`}
                     >
                        {/* Thumbnail */}
                        <div className="w-[50px] h-[64px] rounded-xl overflow-hidden bg-surface-1 border border-border-subtle shrink-0 mr-4 relative">
                          {video.thumbnail_url ? (
                            <img src={video.thumbnail_url} alt="Video thumbnail" className="size-full object-cover" loading="lazy" decoding="async" />
                          ) : (
                            <div className="size-full flex items-center justify-center text-text-secondary bg-zinc-805">
                               <Play className="size-4 text-zinc-650" />
                            </div>
                          )}
                          <div className="absolute inset-x-0 bottom-0 bg-bg-base/60 py-0.5 text-center">
                             <span className="text-[10px] font-mono font-bold text-text-primary/95">#{index+1}</span>
                          </div>
                        </div>

                        {/* Mid Meta */}
                        <div className="flex-1 min-w-0 pr-4.5 flex flex-col justify-center">
                           <p className="text-[13.5px] font-semibold text-text-primary/95 truncate tracking-tight">{video.caption}</p>
                           <div className="flex items-center gap-x-2 mt-1.5 flex-wrap gap-y-1">
                             <div className="px-1.5 py-0.5 rounded bg-surface-2 text-text-secondary font-mono text-[9.5px]">
                        ...
                        <div className="flex flex-col gap-2 shrink-0 pr-1">
                          <button 
                            className="bg-white/5 hover:bg-surface-1 text-text-primary p-1.5 rounded-md transition-colors self-end"
                            onClick={(e) => handleEditVideoClick(e, video.id)}
                            title="Edit Video Options"
                          >
                            <svg className="size-3.5 text-text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                          </button>
                          <div className="grid grid-cols-2 gap-x-3.5 gap-y-1 text-right">
                             <div className="flex items-center justify-end text-[11.5px] text-text-secondary font-mono tracking-wide">
                                <Eye className="size-3.5 mr-1 text-text-secondary" strokeWidth={2} />
                                <span>{video.views >= 1000 ? (video.views/1000).toFixed(1) + 'K' : video.views}</span>
                             </div>
                             <div className="flex items-center justify-end text-[11.5px] text-text-secondary font-mono tracking-wide">
                                <Heart className="size-3.5 mr-1 text-brand-primary fill-brand-primary/10" strokeWidth={2} />
                                <span>{video.likes >= 1000 ? (video.likes/1000).toFixed(1) + 'K' : video.likes}</span>
                             </div>
                             <div className="flex items-center justify-end text-[11.5px] text-text-secondary font-mono tracking-wide">
                                <Bookmark className="size-3.5 mr-1 text-brand-primarymber-500" strokeWidth={2} />
