...
type Tab = 'dashboard' | 'videos' | 'reports' | 'creators' | 'verification' | 'links' | 'spam' | 'analytics' | 'settings' | 'categories';
...
export default function Admin() {
  ...
  async function fetchApplications() {
    try {
      ...
      if (error) console.error("fetchApplications error:", error);
  ...
  async function fetchCreators() {
    try {
      const { data, error } = await supabase
        ...
        .eq('can_upload', true)
        .order('created_at', { ascending: false });
      if (!error && data) setCreators(data as any);
    } catch (err) {}
  }
  ...
  async function fetchBrands() {
    try {
      const { data, error } = await supabase
        ...
        .eq('is_brand', true)
        .order('created_at', { ascending: false });
      if (!error && data) setBrands(data as any);
    } catch (err) {}
  }
  ...
  const handleDismissReport = async (reportId: string) => {
    try {
      ...
      if (!data || data.length === 0) {
        throw new Error('No permission to dismiss. Please apply the latest database.sql to enable Admin RLS policies.');
      }
      fetchReports();
      fetchStats();
    } catch(err: any) {
  ...
  const handleRemoveVerifiedBadge = async (videoId: string) => {
    ...
    try {
      const { data, error } = await supabase.from('videos').update({ is_verified_real: false }).eq('id', videoId).select();
      if (error) throw error;
      if (!data || data.length === 0) {
        throw new Error('No permission or video not found. Please apply the latest database.sql to enable Admin RLS policies.');
      }
      alert('Badge removed.');
      fetchReports();
    } catch (err: any) {
  ...
  const handleSuspendCreator = async (userId: string) => {
    if (!window.confirm('Are you sure? This will delete all their videos.')) return;
    try {
      // Basic suspension implementation for now: delete their videos
      const { data, error } = await supabase.from('videos').delete().eq('user_id', userId).select();
      if (error) throw error;
      if (!data || data.length === 0) {
        throw new Error('No permission or videos not found. Please apply the latest database.sql to enable Admin RLS policies.');
      }
      alert('Creator content removed.');
      fetchReports();
      fetchCreators();
      fetchStats();
    } catch (err: any) {
  ...
  const handleDeleteVideo = async (videoId: string, reportId?: string) => {
    ...
    try {
      // 1. Get video details to find the bunny stream ID if needed
      const { data: videoData } = await supabase.from('videos').select('video_url').eq('id', videoId).single();
      
      // 2. Delete from DB
      const { data, error } = await supabase.from('videos').delete().eq('id', videoId).select();
      if (error) throw error;
      if (!data || data.length === 0) {
        throw new Error('No permission or video not found. Please apply the latest database.sql to enable Admin RLS policies.');
      }
      
      // 3. Attempt Bunny Stream delete
  ...
  const handleRevokeUpload = async (userId: string) => {
    if (!window.confirm('Revoke upload privileges for this user?')) return;
    await supabase.from('profiles').update({ can_upload: false }).eq('id', userId);
    fetchCreators();
  };
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100dvh-64px-env(safe-area-inset-bottom))] w-full bg-black text-white pt-safe flex flex-col font-sans relative">
        ...
        <div className="max-w-md mx-auto w-full flex-1 flex flex-col relative z-10">
          <div className="px-4 py-6 sticky top-0 z-10">
            ...
            <div className="flex space-x-1 bg-white/5 p-1 rounded-2xl backdrop-blur-md">
              {[1, 2, 3, 4, 5].map((i) => (
  ...
  if (isAdmin === false) {
    return (
      <div className="min-h-[calc(100dvh-64px-env(safe-area-inset-bottom))] w-full bg-black text-white flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-red-500/10 blur-[100px] rounded-full" />
        <ShieldAlert className="w-16 h-16 text-red-500 mb-6 drop-shadow-[0_0_15px_rgba(239,68,68,0.5)] z-10" />
        ...
        <button onClick={() => navigate('/')} className="px-8 py-3.5 bg-white text-black rounded-xl font-bold tracking-wide active:scale-95 transition-transform z-10">
          Return Home
        </button>
      </div>
    );
    ...
    <div className="flex h-screen bg-[#0E0E10] text-[#E0E0E0] font-sans overflow-hidden">
      ...
      <div className={cn(
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-white/5 shrink-0 gap-3">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="p-1 -ml-2 text-zinc-400 hover:text-white transition-colors">
            ...
            <div className="w-8 h-8 rounded-full bg-[#F97316] text-white flex items-center justify-center font-bold text-xl">G</div>
          ...
          <button className="md:hidden p-1 text-zinc-400 hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen bg-[#0A0A0C] w-full relative overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto w-full p-4 md:p-8">
          <div className="max-w-[1400px] w-full mx-auto space-y-4 md:space-y-6">
                 ...
                 <div className="bg-[#161619] border border-white/5 rounded-2xl p-5 flex flex-col justify-between">
                    ...
                    <div className="flex items-end justify-between">
                       ...
                       <div className="flex flex-col items-end">
                          <TrendingUp className="w-5 h-5 text-zinc-500 mb-1" />
                    ...
                    <div className="flex flex-col gap-4">
                       {reports.slice(0, 5).map((r, i) => (
                         <div key={r.id} className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                               <div className="w-4 h-4 rounded-full flex items-center justify-center border-2 border-[#161619] ring-2 ring-[#EF4444]" />
                          ...
                          {reports.slice(0, 10).map((r) => (
                             <tr key={r.id} className="hover:bg-white/[0.02] group">
                                ...
                                <td className="py-4 px-5 text-right flex justify-end gap-2">
                                   <button className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/10"><Eye className="w-4 h-4 text-zinc-400"/></button>
                                   <button className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center hover:bg-white/10"><CheckCircle className="w-4 h-4 text-zinc-400"/></button>
                                </td>
                             </tr>
                          ))}
                          {reports.length === 0 && (
                          ...
                          {videos.map((v) => (
                             <tr key={v.id} className="hover:bg-white/[0.02] group">
                                <td className="py-4 px-5 flex items-center gap-3 text-white">
                                   {v.thumbnail_url ? (
                                      <img src={v.thumbnail_url} className="w-10 h-10 rounded-md object-cover" />
                                   ) : (
                                ...
                                <td className="py-4 px-5">
                                   <div className="flex items-center gap-2">
                                     {v.profiles?.avatar_url ? (
                                        <img src={v.profiles.avatar_url} className="w-6 h-6 rounded-full object-cover" />
                                     ) : (
                                ...
                                <td className="py-4 px-5 text-right flex justify-end gap-2">
                                   <button className="w-8 h-8 rounded-lg border border-white/10 flex items-center justify-center hover:bg-white/10 bg-black/20"><Eye className="w-4 h-4 text-zinc-400"/></button>
                                   <button className="w-8 h-8 rounded-lg border border-white/10 flex items-center justify-center hover:bg-white/10 bg-black/20"><CheckCircle className="w-4 h-4 text-zinc-400"/></button>
                                   <button className="w-8 h-8 rounded-lg border border-white/10 flex items-center justify-center hover:bg-white/10 bg-black/20"><XCircle className="w-4 h-4 text-zinc-400"/></button>
                                </td>
                             </tr>
                          ))}
                          {videos.length === 0 && (
                             ...
                             <tr key={app.id} className="hover:bg-white/[0.02] group">
                                <td className="py-4 px-5 flex items-center gap-3 text-white">
                                   {app.profiles?.avatar_url ? (
                                      <img src={app.profiles.avatar_url} className="w-10 h-10 rounded-full object-cover" />
                                   ) : (
                                ...
                                <td className="py-4 px-5">
                                   <span className={cn(
                                      "px-4 py-1.5 rounded-xl border font-medium bg-black/40",
                                ...
                                <td className="py-4 px-5 text-right flex justify-end gap-2">
                                   <button className="w-8 h-8 rounded-lg border border-white/10 bg-black/20 flex items-center justify-center hover:bg-white/10"><Eye className="w-4 h-4 text-zinc-400"/></button>
                                   <button className="w-8 h-8 rounded-lg border border-white/10 bg-black/20 flex items-center justify-center hover:bg-white/10"><CheckCircle className="w-4 h-4 text-zinc-400"/></button>
                                </td>
                             </tr>
                          ))}
          ...
          {activeTab === 'categories' && (() => {
            ...
            return (
            <div className="space-y-5 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl">
              <div className="bg-[#161619] border border-[#F97316]/20 rounded-2xl overflow-hidden shadow-[0_0_20px_rgba(249,115,22,0.05)]">
                 <div className="p-5 border-b border-white/5 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                  ...
                  <div className="w-full sm:w-auto relative">
                     ...
                     <input
                       ...
                       className="w-full sm:w-[250px] bg-black/40 text-white placeholder-zinc-500 rounded-xl pl-9 pr-4 py-2 text-[14px] focus:outline-none focus:ring-1 focus:ring-blue-500 border border-white/5"
                     />
                  </div>
                 </div>
                 ...
                 <div className="p-5 border-b border-white/5 bg-[#131316]">
                    <form onSubmit={handleCreateCategory} className="flex gap-3">
                      <input
                        ...
                        className="flex-1 bg-black/40 text-white placeholder-zinc-500 rounded-xl px-4 py-3 text-[14px] focus:outline-none focus:ring-1 focus:ring-blue-500 border border-white/5"
                      />
                      <button
                        type="submit"
          ...
          {['reports', 'creators', 'links', 'spam', 'analytics', 'settings'].includes(activeTab) && (
            <div className="py-12 text-center text-zinc-500 animate-in fade-in h-[300px] flex items-center justify-center rounded-2xl border border-white/5 bg-[#161619]">
              <div>
                <Layers className="w-12 h-12 mx-auto mb-4 opacity-20" />
                <p className="tracking-wide">This section ({activeTab}) is under construction.</p>
              </div>
            </div>
          )}
          </div>
      ...
      {editingCategory === cat.id ? (
        <div className="flex flex-col sm:flex-row flex-1 items-start sm:items-center gap-3 w-full">
          <input
            ...
            className="bg-black/40 text-white rounded-lg px-3 py-2 text-[14px] focus:outline-none border border-white/10 focus:border-blue-500 w-full sm:flex-1 sm:max-w-[200px]"
            onKeyDown={(e) => {
          ...
          <div className="w-full sm:flex-1 sm:max-w-[250px] flex items-center gap-2">
            <input
              ...
              className="bg-black/40 text-white rounded-lg px-3 py-2 text-[14px] focus:outline-none border border-white/10 focus:border-blue-500 w-full"
            />
            <label className="cursor-pointer p-2 text-zinc-400 hover:text-white rounded-lg transition-all h-full bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
              {isUploadingCategoryImage ? <Loader2 className="w-4 h-4 animate-spin" /> : <ImagePlus className="w-4 h-4" />}
          ...
          <div className="flex items-center gap-3">
            {onSelect && (
              <input
                ...
                className="w-4 h-4 rounded border-white/20 bg-black/50 text-blue-500 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer"
              />
            )}
            <button
              {...attributes}
            ...
            {cat.image_url ? (
              <img src={cat.image_url} alt={cat.name} className="w-10 h-10 rounded-md object-cover border border-white/10 mx-1 shrink-0 bg-white/5" />
            ) : (
              <div className="w-10 h-10 rounded-md bg-white/5 border border-white/10 mx-1 shrink-0 flex items-center justify-center">
