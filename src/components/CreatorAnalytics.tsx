  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-surface-1/95 backdrop-blur-md border border-border-subtle p-3 rounded-xl shadow-xl gap-y-1.5 font-sans min-w-[125px]">
        {title && (
          <p className="text-[11px] font-bold text-text-secondary uppercase tracking-widest truncate max-w-[150px]">{title}</p>
        )}
...
export default function CreatorAnalytics({ videos, engagementDetails }: CreatorAnalyticsProps) {
  ...
  return (
    <div className="p-4 bg-surface-2 border border-border-subtle rounded-3xl shadow-lg flex flex-col my-4">
      <div className="flex flex-col mb-4">
        <h3 className="text-[16px] font-bold text-text-primary tracking-wide">Summary Analytics</h3>
        <p className="text-[12.5px] font-medium text-emerald-400 mt-1">Real-time engagement for recent posts</p>
      </div>

      <div className="grid grid-cols-3 gap-2 mb-6">
        <div className="flex flex-col p-3 bg-surface-1 border border-border-subtle rounded-2xl">
           <span className="text-[11px] font-bold text-text-secondary uppercase tracking-wider mb-1">Views</span>
           <span className="text-[18px] font-sans font-bold text-text-primary tracking-tight">{totalViews >= 1000 ? (totalViews/1000).toFixed(1)+'K' : totalViews}</span>
        </div>
        <div className="flex flex-col p-3 bg-surface-1 border border-border-subtle rounded-2xl">
           <span className="text-[11px] font-bold text-text-secondary uppercase tracking-wider mb-1">Likes</span>
           <span className="text-[18px] font-sans font-bold text-text-primary tracking-tight">{totalLikes >= 1000 ? (totalLikes/1000).toFixed(1)+'K' : totalLikes}</span>
        </div>
        <div className="flex flex-col p-3 bg-surface-1 border border-border-subtle rounded-2xl">
           <span className="text-[11px] font-bold text-text-secondary uppercase tracking-wider mb-1">Eng. Rate</span>
           <span className="text-[18px] font-sans font-bold text-text-primary tracking-tight">{engagementRate}%</span>
        </div>
      </div>

      <div className="h-[260px] w-full text-text-secondary text-xs mt-2">
