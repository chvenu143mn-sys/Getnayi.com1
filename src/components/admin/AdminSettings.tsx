    ...
    <div className="gap-y-6 animate-in fade-in duration-500">
      ...
      <form onSubmit={handleSaveSettings} className="gap-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <button aria-label="button" 
                ...
                className={cn(
                  ...
                  settings.require_verification_to_upload 
                    ...
                    : "bg-zinc-800 border-white/5 justify-start"
                )}
              >
          ...
          <div className="bg-[#141416] border border-white/5 rounded-2xl p-5 gap-y-4">
            ...
            <div className="gap-y-1.5">
              <label className="text-zinc-400 text-[10.5px] font-mono uppercase tracking-wider block">Administrator Support Mail</label>
              <input
                type="email"
                required
                value={settings.support_email || ''}
                onChange={(e) => setSettings((prev: any) => ({ ...prev, support_email: e.target.value }))}
                className="w-full bg-[#0c0c0e]/45 border border-white/5 focus:border-white/10 p-2.5 rounded-xl text-xs text-white placeholder-zinc-550 focus:outline-none focus:ring-1 focus:ring-zinc-800"
            ...
            <div className="gap-y-1.5">
              <label className="text-zinc-400 text-[10.5px] font-mono uppercase tracking-wider block">Max Allocation Upload limit (MB)</label>
              <input
                type="number"
                required
        ...
        <div className="bg-[#141416] border border-white/5 rounded-2xl p-5 gap-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="gap-y-3">
              <label className="text-zinc-400 text-xs font-semibold block">Trusted Outbound Store Roots (Whitelist)</label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-[#0c0c0e]/45 border border-white/5 focus:border-white/10 px-3 py-2 rounded-xl text-xs text-white"
                />
                <button aria-label="button" 
                  type="button"
                  onClick={handleAddAllowed}
                  className="p-2 bg-white/5 hover:bg-white/10 text-white rounded-xl border border-white/10"
                >
              ...
              {/* Badges container */}
              <div className="flex flex-wrap gap-1.5 p-3 bg-[#0c0c0e]/20 border border-white/5 rounded-xl min-h-[85px] align-content-start">
                {(settings.allowed_product_domains || []).map((tag: string) => (
                  <span key={tag} className="flex items-center gap-1.5 bg-green-500/10 border border-green-500/20 text-green-500 px-2.5 py-1 text-[11px] font-mono font-semibold rounded-lg">
                    {tag}
                    <button aria-label="button"  type="button" onClick={() => handleRemoveAllowed(tag)} className="text-green-500 hover:text-white">&times;</button>
                  </span>
                ))}
            ...
            {/* Blacklist list */}
            <div className="gap-y-3">
              <label className="text-zinc-400 text-xs font-semibold block">Suspicious Outbound Store Roots (Blocklist)</label>
              <div className="flex items-center gap-2">
                <input
                  type="text"
              ...
              <div className="flex flex-wrap gap-1.5 p-3 bg-[#0c0c0e]/20 border border-white/5 rounded-xl min-h-[85px] align-content-start">
                {(settings.blacklisted_product_domains || []).map((tag: string) => (
                  <span key={tag} className="flex items-center gap-1.5 bg-red-500/10 border border-red-500/20 text-red-500 px-2.5 py-1 text-[11px] font-mono font-semibold rounded-lg">
                    {tag}
                    <button aria-label="button"  type="button" onClick={() => handleRemoveBlacklist(tag)} className="text-red-500 hover:text-white">&times;</button>
                  </span>
                ))}
        ...
        <div className="flex items-center justify-end">
          <button aria-label="button" 
            type="submit"
            className="flex items-center gap-2 px-6 py-3 bg-[#F97316] hover:bg-orange-600 active:scale-95 text-white text-sm font-bold rounded-xl shadow-lg transition"
          >
